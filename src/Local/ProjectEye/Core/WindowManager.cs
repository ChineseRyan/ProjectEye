...
namespace ProjectEye.Core
{
    ...
    public class WindowManager
    {
        ...
        private static Window CreateWindow(string name, string screen, double left = -999999, double top = -999999, double width = -999999, double height = -999999, bool newViewModel = false)
        {
            ...
            if (type == null)
            {
                LogHelper.Error($"WindowManager.CreateWindow: 找不到窗口类型 ProjectEye.Views.{name}");
                return null;
            }
            Window objWindow = (Window)type.Assembly.CreateInstance(type.FullName);
            if (objWindow == null)
            {
                LogHelper.Error($"WindowManager.CreateWindow: 无法创建窗口实例 {type.FullName}");
                return null;
            }
            objWindow.Uid = name;
        ...
        public static void Show(string name)
        {
            ...
            {
                LogHelper.Error($"WindowManager.Show({name}): " + ex.ToString());
            }
        }
        #endregion
        ...
        public static int Close(string name)
        {
            ...
            foreach (var window in windows)
            {
                ...
                {
                    LogHelper.Error($"WindowManager.Close: 关闭窗口 {name} 失败: " + ex.ToString());
                }
            }
        ...
        public static int Hide(string name)
        {
            ...
            foreach (var window in windows)
            {
                ...
                {
                    LogHelper.Error($"WindowManager.Hide: 隐藏窗口 {name} 失败: " + ex.ToString());
                }
            }
            return windows.Length;
        }
        ...
        private static object CreateViewModel(string windowName)
        {
            try
            {
                ...
                var constructorInfoObj = type.GetConstructors();
                if (constructorInfoObj == null || constructorInfoObj.Length == 0)
                {
                    LogHelper.Error($"WindowManager.CreateViewModel: 找不到 {viewModelName} 的构造函数");
                    return null;
                }
                var constructorParameters = constructorInfoObj[0].GetParameters();
                ...
                if (ctor == null)
                {
                    LogHelper.Error($"WindowManager.CreateViewModel: 找不到 {viewModelName} 匹配的构造函数");
                    return null;
                }
            ...
            {
                LogHelper.Error($"WindowManager.CreateViewModel({windowName}): " + ex.ToString());
                return null;
            }
        }
        #endregion
